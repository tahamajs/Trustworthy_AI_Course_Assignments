...
The plotting logic is explicitly tied to report requirements through fixed filenames and deterministic ordering. In tabular generation, the expanded pipeline exports class distribution, model learning curves, confusion-matrix comparison, ROC/PR comparison, calibration curves, LIME-SHAP agreement diagnostics, three local explanation figures for stable test indices \([0,1,2]\), and NAM feature-response functions; this progression intentionally moves from global data/optimization behavior to local explanation behavior. In vision generation, synthetic \(224\times224\) RGB inputs are used to guarantee fully local execution with no external assets, and the expanded outputs include Grad-CAM heatmaps, Grad-CAM overlays, Guided Grad-CAM composition, SmoothGrad-vs-guided comparison, and SmoothGrad sample-count sweep diagnostics. All metrics and stability statistics are serialized to \texttt{report/figures/metrics\_summary.json}, creating a machine-readable traceability layer between generated artifacts and manuscript claims.
...
The expanded quantitative view shows that MLP remains stronger than NAM across thresholded and threshold-free discrimination metrics while NAM stays competitively close with higher structural interpretability, and the lower MLP Brier score indicates modestly better probability calibration quality; additionally, high SmoothGrad cosine similarities across sample counts indicate stable saliency structure under increased Monte Carlo averaging, which supports using larger \(K\) for smoother yet semantically consistent visual explanations.
...
\caption{SmoothGrad maps for \(K=5\), \(K=20\), and \(K=50\).}
\label{fig:smoothgrad_sweep}
\end{figure}
The SmoothGrad sweep plot demonstrates convergence behavior as sample count \(K\) increases: the \(K=5\) map retains more stochastic texture, while \(K=20\) and \(K=50\) progressively stabilize major relevance regions with reduced speckle, and this qualitative trend is quantitatively supported by high cosine similarities (notably stronger between \(K=20\) and \(K=50\) than between \(K=5\) and \(K=20\)); theoretically this matches Monte Carlo estimator convergence, where variance decays with additional perturbation samples and saliency structure approaches a stable expectation under the noise model.

\section{Discussion}
The complete expanded figure set supports a consistent interpretation narrative with stronger theoretical grounding: tabular diagnostics now cover optimization dynamics, thresholded and threshold-free discrimination, calibration quality, and local attribution agreement, while vision diagnostics progress from localization to overlay validation, guided fusion, and Monte Carlo stability analysis.
