...
\texttt{losses.py} contains two loss modules. \texttt{LabelSmoothingCrossEntropy} constructs a softened class distribution and computes expected negative log likelihood under that distribution, reducing overconfidence. \texttt{CircleLoss} constructs pairwise similarity matrices on normalized embeddings, forms positive and negative masks, computes adaptive weighting terms (\(a_p\), \(a_n\)), and uses log-sum-exp aggregation before softplus. Circle Loss is integrated into training via \texttt{train.py --loss circle}; the training loop calls \texttt{model(x, return\_features=True)} and minimizes \texttt{CircleLoss(features, labels)} on the 512-d embeddings.
...
\subsection{Module Input/Output Contract}
Table~\ref{tab:moduleio} summarizes the I/O and side effects of each core module. This makes the pipeline auditable by showing exactly which artifacts are written and which inputs are consumed at each stage.

\begin{table}[t]
\caption{Module-level input/output contract}
\label{tab:moduleio}
\centering
\footnotesize
...
\midrule
\texttt{datasets.py} & dataset name, batch size, augment flag & dataloaders, class count, channel count \\
\texttt{models/resnet18\_custom.py} & input tensors, BN flag & logits, optional features \\
\texttt{losses.py} & logits + labels & loss scalar (CE/LS/Circle) \\
\texttt{attacks.py} & model, inputs, \(\epsilon,\alpha\) & adversarial inputs (clipped) \\
\texttt{train.py} & args, dataloaders & checkpoints, history CSV/JSON, training curves \\
\texttt{eval.py} & checkpoint, dataloader & plots, metrics JSON, sweeps \\
\texttt{run\_report\_pipeline.py} & args & copied report figures + metrics summary \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Dataset Statistics and Transform Contract}
Table~\ref{tab:datastats} records the data-level contract enforced by \texttt{datasets.py}: image size, channels, class count, and normalization statistics. Explicitly stating these values is important because they determine the scale and conditioning of the optimization problem and must be consistent across training and evaluation to avoid subtle distribution shifts.

\begin{table}[t]
\caption{Dataset statistics and preprocessing contract}
\label{tab:datastats}
\centering
\footnotesize
\begin{tabular}{lcccc}
...
\midrule
SVHN & \(32\times32\) & 3 & 10 & (0.4377, 0.4438, 0.4728) / (0.1980, 0.2010, 0.1970) \\
MNIST & \(32\times32\) & 1\(\rightarrow\)3 & 10 & (0.1307) / (0.3081) \\
CIFAR10 & \(32\times32\) & 3 & 10 & (0.4914, 0.4822, 0.4465) / (0.2470, 0.2435, 0.2616) \\
Fallback FakeData & \(32\times32\) & 3 & 10 & inherits SVHN/MNIST transform \\
\bottomrule
\end{tabular}
\end{table}
...
SVHN-LS & \texttt{train.py --dataset svhn --label-smoothing 0.25 --epochs 1 --demo} \\
SVHN-Adam & \texttt{train.py --dataset svhn --optimizer adam --lr 1e-3 --epochs 1 --demo} \\
SVHN-Pretrained & \texttt{train.py --dataset svhn --pretrained imagenet --epochs 1 --demo} \\
SVHN-Augment & \texttt{train.py --dataset svhn --augment-preset digits\_safe --epochs 1 --demo} \\
MNIST-Train & \texttt{train.py --dataset mnist --epochs 1 --demo} \\
MNIST-Finetune & \texttt{train.py --finetune-from .../best.pth --finetune-samples 750 --freeze-conv --demo} \\
CIFAR-Base & \texttt{train.py --dataset cifar10 --cifar-split 0.2 --epochs 1 --demo} \\
CIFAR-FGSM-AT & \texttt{train.py --dataset cifar10 --cifar-split 0.2 --adv-train --attack fgsm --epochs 1 --demo} \\
CIFAR-PGD-AT & \texttt{train.py --dataset cifar10 --cifar-split 0.2 --adv-train --attack pgd --iters 7 --epochs 1 --demo} \\
CIFAR-Circle & \texttt{train.py --dataset cifar10 --cifar-split 0.2 --loss circle --epochs 1 --demo} \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:robustness} summarizes robustness outputs from \texttt{checkpoints/report\_summary/robustness\_summary.csv}, which is generated by \texttt{run\_full\_hw1.py}. Columns are clean accuracy, FGSM at \(\epsilon=0.1\), FGSM at \(8/255\), PGD, and random noise. In full-data settings these columns separate clean and adversarial performance; under demo/fallback data they serve as a pipeline-verification baseline.
...
\toprule
Model & Clean & FGSM (\(\epsilon{=}0.1\)) & FGSM (\(8/255\)) & PGD & Noise \\
\midrule
CIFAR Base (A) & 12.11 & 12.11 & 12.11 & 12.11 & 12.11 \\
CIFAR + FGSM AdvTrain (B) & 12.11 & 12.11 & 12.11 & 12.11 & 12.11 \\
CIFAR + PGD AdvTrain & 12.11 & 12.11 & 12.11 & 12.11 & 12.11 \\
CIFAR + Circle Loss (D) & 12.11 & 12.11 & 12.11 & 12.11 & 12.11 \\
\bottomrule
\end{tabular}
\end{table}

Adversarial examples for the report are generated with FGSM at \(\epsilon=0.1\) and random pixel noise via \texttt{eval.py --save-grid --epsilon 0.1}; the resulting figure is suitable for the assignment requirement to ``display some of these images.'' Comparison of (A) baseline, (B) augmented training with 50\% adversarial perturbation, and (D) Circle Loss training is in this table; (C) is the Circle Loss theory subsection.
...
For digit datasets, label-preserving transformations are constrained: small translations, mild contrast changes, and limited geometric jitter can be beneficial, but strong rotations, vertical flips, and aggressive perspective warps may change class identity (e.g., \texttt{6} vs. \texttt{9}) or create out-of-manifold artifacts. In particular, \textbf{horizontal flip} can swap 6 and 9 and is therefore unsuitable unless disabled. The implementation supports a \texttt{digits\_safe} preset (\texttt{--augment-preset digits\_safe}) that omits horizontal flip and keeps random crop and color jitter. The default preset retains horizontal flip for compatibility with generic RGB datasets. Therefore, augmentation policy should be data-semantic rather than generic.
...
\subsection{Structured Error Taxonomy}
For deployment-oriented interpretation, errors are categorized by operational impact. \textbf{False positives} correspond to spurious alarms and can lead to unnecessary interventions, while \textbf{false negatives} correspond to missed detections and can be safety-critical in high-stakes contexts. \textbf{Misranked but top-k-correct} cases indicate that the model retains partial evidence but lacks decisive boundaries, suggesting that reranking or calibration may improve outcomes without full retraining. \textbf{Overconfident errors} indicate a mismatch between uncertainty and correctness and are particularly harmful because they undermine selective prediction and human override policies. This taxonomy connects the PRF1, top-k, and calibration diagnostics into a single actionable framework for triaging model improvements.
...
Label smoothing experiment (smoothing=0.25) & Complete & \texttt{losses.py}, \texttt{--label-smoothing 0.25}, Table II \\
Optimizer comparison (SGD/Adam) & Complete & Table II (SVHN baseline vs Adam) \\
Data augmentation (digits\_safe preset) & Complete & \texttt{datasets.py} \texttt{augmentation\_preset}, Sec. VII \\
Pretrained ResNet18 (ImageNet) & Complete & \texttt{models/resnet18\_pretrained.py}, \texttt{--pretrained imagenet} \\
Reverse training (MNIST$\rightarrow$SVHN) & Complete & Table III (MNIST Train eval on SVHN) \\
Fine-tune classifier (750 SVHN, freeze conv) & Complete & \texttt{--finetune-from}, \texttt{--freeze-conv}, \texttt{--finetune-samples 750} \\
Cross-domain transfer (both directions) & Complete & \texttt{--eval-datasets svhn,mnist}, Table III + Sec. V-B \\
FGSM (\(\epsilon=0.1\)) and random noise display & Complete & \texttt{eval.py --save-grid --epsilon 0.1}, Fig. adv\_examples \\
FGSM and PGD robustness evaluation & Complete & \texttt{eval.py} attacks + Table IV + Fig. 8 \\
Adversarial training variants (A/B/D) & Complete & Table I and IV (CIFAR base, FGSM-AT, PGD-AT, Circle) \\
Circle Loss theory (C) and training (D) & Complete & \texttt{losses.py}, \texttt{--loss circle}, Sec. III and Sec. VII \\
...
\subsection{Parameter Sensitivity Mini-Appendix}
Even under identical data, model quality can change significantly with hyperparameters. Learning rate controls the effective step size and can cause underfitting (too small) or instability (too large). Momentum and weight decay adjust optimization geometry and implicit regularization, affecting both generalization and adversarial margin. Label smoothing changes gradient distribution and thus calibration behavior. Attack parameters \(\epsilon\), \(\alpha\), and iteration count determine the strength of the inner maximization and can change robustness conclusions by several percentage points. For this reason, all primary parameters are reported explicitly in Tables~\ref{tab:trainparams} and~\ref{tab:params}, and robustness claims are paired with sweep-based diagnostics rather than single-point numbers.
...
\textbf{One-command report artifact pipeline (single dataset)}
\begin{verbatim}
cd code && python run_report_pipeline.py --epochs 3
\end{verbatim}

\textbf{Full HW1 pipeline (all experiments + summary CSVs)}
\begin{verbatim}
cd code && python run_full_hw1.py --demo
\end{verbatim}
This writes \path{checkpoints/report_summary/training_summary.csv}, \path{cross_domain_summary.csv}, \path{robustness_summary.csv} and copies figures to \path{../report/figures}. Use \texttt{--no-demo} and \texttt{--epochs 80} for full dataset runs.

\textbf{Cross-domain evaluation (e.g.\ SVHN model on SVHN and MNIST)}
\begin{verbatim}
python eval.py --checkpoint checkpoints/svhn_baseline/best.pth \
  --eval-datasets svhn,mnist --cross-eval-csv checkpoints/svhn_baseline/cross_eval.csv --dataset svhn
\end{verbatim}
...
\begin{itemize}[leftmargin=1.2em]
  ...
  \item \path{report/figures/adv_examples_eps01.png} (FGSM \(\epsilon=0.1\))
