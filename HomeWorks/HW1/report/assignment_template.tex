...
We report results for: (a) standard Cross-Entropy, (b) Label-Smoothing Cross-Entropy (smoothing=0.1), and (c) Circle Loss (when used for robustness experiments). Implementation notes: label smoothing is applied on the logits level before cross-entropy.
...
7. Robustness experiments (FGSM / PGD) on separate CIFAR-10 setup (Part 2).
...
\begin{table}[H]
  \centering
  \caption{Representative results (replace with your measured values)}
  \begin{tabular}{lccc}
    \toprule
